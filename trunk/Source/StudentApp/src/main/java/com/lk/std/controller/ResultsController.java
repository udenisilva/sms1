...
public class ResultsController {
	...
	public ModelAndView getHomePage(HttpServletRequest request) {
		...
		try {
			if (!StringUtils.isBlank(request.getParameter("subject"))) {
				...
				sub = subjectService.findById(subject);
		...
		try {
			if (!StringUtils.isBlank(request.getParameter("year"))) {
				...
				marks = marksService.findByGradeAndSubjectAndAcademicyear(grade, sub, year);
				modelMap.addAttribute("subjectId", subject);
				modelMap.addAttribute("marks", marks);
				modelMap.addAttribute("reason", marks.get(0).getExaminationType());
	...
	public String StaffPager(HttpServletRequest request, @ModelAttribute("marksDto") MarksDto marksDto,
		...
		// } else {
		// return "redirect:studentattendance.htm?" +
		// ApplicationConstants.MESSAGE + "=" + ApplicationConstants.ERROR;
		// }

		return "redirect:results.htm?" + ApplicationConstants.MESSAGE + "=" + ApplicationConstants.ERROR;
	}
